...
function proposalVisualStorageKey(kind) { return `GetEV-proposal-${activeBidId || 'default'}-${copiedProposal?.copyId || 'base'}-${kind}`; }
...
function positionProposalLogoOverlay() { const photo=$('#proposalPhoto'), card=$('#proposalLogo')?.closest('.proposal-logo-card'); if (!photo || !card || !photo.classList.contains('proposal-photo-custom')) return; const bounds=photo.getBoundingClientRect(), parent=photo.parentElement?.getBoundingClientRect(); if (!parent || !photo.naturalWidth || !photo.naturalHeight) return; const ratio=photo.naturalWidth/photo.naturalHeight, width=Math.min(bounds.width,bounds.height*ratio), height=width/ratio, left=(bounds.width-width)/2, top=0; card.style.left=`${Math.max(12,left+24)}px`; card.style.top=`${Math.max(12,top+height-card.offsetHeight-24)}px`; card.style.bottom='auto'; }
function applyProposalVisual(kind, imageUrl) { const target = kind === 'photo' ? $('#proposalPhoto') : $('#proposalLogo'); if (target && imageUrl) { target.src = imageUrl; if (kind === 'photo') { target.classList.toggle('proposal-photo-custom', /^data:image\//i.test(imageUrl) || !/ArcGIS\/rest\/services\/World_Imagery/i.test(imageUrl)); target.addEventListener('load', positionProposalLogoOverlay, { once:true }); } if (kind === 'logo') target.closest('.proposal-logo-card')?.style.setProperty('display',''); } applyProposalVisualSettings(kind); if (kind === 'logo') setTimeout(positionProposalLogoOverlay, 0); }
window.addEventListener('resize', positionProposalLogoOverlay);
...
function openNewProposalSetup() {
  ...
  if (!modal) {
    ...
    modal.addEventListener('click', event => { if (event.target === modal) modal.hidden = true; });
     modal.querySelector('form').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const city = String(data.get('city')).trim() || 'Lehi'; const stateCode = String(data.get('stateCode')).trim().toUpperCase() || 'UT'; const street = String(data.get('street')).trim() || 'Address to be confirmed'; const zip = String(data.get('zip')).trim() || '84043'; const siteName = String(data.get('siteName')).trim() || `Proposed EV charging site in ${city}`; const location = [street, city, stateCode, zip].join(', '); const templateBidId = proposalTemplateBidId; const selectedScopes = { ev: data.get('ev') === 'on', solar: data.get('solar') === 'on', storage: data.get('storage') === 'on' }; const scopes = Object.values(selectedScopes).some(Boolean) ? selectedScopes : { ev: true, solar: false, storage: false }; const locationType = String(data.get('locationType') || '').trim(); const otherType = String(data.get('locationTypeOther') || '').trim(); const newProposalState = cloneProposalTemplateState(); const typedLocationResults = await searchLocationSuggestions(location); const typedLocation = typedLocationResults.find(result => Number.isFinite(Number(result?.lat)) && Number.isFinite(Number(result?.lon))) || typedLocationResults[0]; newProposalState.overview = { ...newProposalState.overview, siteName, proposalName: `${siteName} ${city}, ${stateCode}`, location, city, stateCode, locationType: locationType === 'Other' ? (otherType || 'Commercial business') : (locationType || inferNewProposalLocationType(siteName)), proposalDate: String(data.get('proposalDate')) || new Date().toISOString().slice(0, 10), status: 'Draft' }; newProposalState.site.latitude = Number(form.dataset.selectedLat) || Number(typedLocation?.lat) || 40.391617; newProposalState.site.longitude = Number(form.dataset.selectedLon) || Number(typedLocation?.lon) || -111.849055; newProposalState.ev = { ...newProposalState.ev, locationDataStatus: 'pending', trafficSource: 'Location lookup pending', trafficSourceRecordId: '', trafficRoadName: '', dailyTraffic: 0, travelRouteDistance: 0, marketProofVisitsPerDay: 0, marketProofPorts: 0, marketProofSessions3m: 0, observedCharges3m: 0, sourceWindow: 'Location-specific data pending' }; newProposalState.brand = { ...state.brand }; const copyId = `proposal-${Date.now()}`; newProposalState.overview.sitePhoto = await readProposalImage(form.elements.sitePhoto?.files?.[0]); newProposalState.overview.siteLogo = await readProposalImage(form.elements.siteLogo?.files?.[0]); try { if (newProposalState.overview.sitePhoto) localStorage.setItem(`GetEV-proposal-${templateBidId}-${copyId}-photo`, newProposalState.overview.sitePhoto); if (newProposalState.overview.siteLogo) localStorage.setItem(`GetEV-proposal-${templateBidId}-${copyId}-logo`, newProposalState.overview.siteLogo); } catch {} const proposalUrl = new URL(window.location.href); proposalUrl.hash = ''; proposalUrl.search = ''; proposalUrl.searchParams.set('bid', templateBidId); proposalUrl.searchParams.set('copy', encodeCopyPayload({ version: 1, copyId, bidId: templateBidId, scopes, inlineEdits: {}, state: newProposalState })); window.location.assign(proposalUrl.toString()); });
  }
  ...
  form.querySelectorAll('input[name="street"],input[name="city"],input[name="stateCode"],input[name="zip"]').forEach(input => input.addEventListener('input', () => { delete form.dataset.selectedLat; delete form.dataset.selectedLon; }));
