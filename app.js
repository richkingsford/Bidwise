...
const paintRange = input => { const min = Number(input.min), max = Number(input.max), value = Number(input.value); const pct = max === min ? 0 : Math.max(0, Math.min(100, ((value - min) / (max - min)) * 100)); const control = input.parentElement; const fill = control.querySelector('.config-range-fill'); const track = control.querySelector('.config-range-track'); if (track) { const width = Math.max(0, input.getBoundingClientRect().width - 18); track.style.width = `${width}px`; } if (fill) fill.style.width = `${pct}%`; };
