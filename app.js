...
function addLayoutObject(type, latlng) {
  ...
  layoutObjects.push(item); marker.bindTooltip(type, { direction: 'top', offset: [0, -12] }); marker.bindPopup(`<b>${esc(type)}</b><br><span class="layout-popup-angle">Rotation: 0°</span><br><button type="button" class="layout-rotate-button" data-rotate="-15">↺ 15°</button> <button type="button" class="layout-rotate-button" data-rotate="15">↻ 15°</button>`); marker.on('popupopen', event => { const popup = event.popup.getElement(); popup.querySelectorAll('.layout-rotate-button').forEach(button => button.addEventListener('click', () => { rotate(Number(button.dataset.rotate)); popup.querySelector('.layout-popup-angle').textContent = `Rotation: ${item.rotation}°`; })); }); marker.on('dragend', () => { const pos = marker.getLatLng(); item.lat = pos.lat; item.lng = pos.lng; refreshLayoutSummary(); }); refreshLayoutSummary(); setLayoutStatus(`${type} placed · click it to rotate, or drag it to refine the location`);
}
