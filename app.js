...
window.addEventListener('getev:bulk-proposals', event => { const businesses = event.detail?.businesses || []; const copies = readLocalBids(); businesses.filter(business => { const address = business.address || {}; return address.house_number || address.road || business.display_name?.includes(','); }).slice(0, 10).forEach((business, index) => { const address = business.address || {}; const name = business.namedetails?.name || business.name || `Business prospect ${index + 1}`; const city = address.city || address.town || address.village || address.municipality || 'Local market'; const stateCode = locationStateCode(address, city) || 'UT'; const street = [address.house_number, address.road].filter(Boolean).join(' ') || 'Address to be confirmed'; const zip = address.postcode || '84043'; const id = `bulk-${Date.now()}-${index}`; copies[id] = { label: name, locationLabel: `${street}, ${city}, ${stateCode} ${zip}`, sourceBidId: 'kneaders-orem', overrides: { overview: { siteName: name, proposalName: `${name} ${city}, ${stateCode}`, location: `${street}, ${city}, ${stateCode} ${zip}`, city, stateCode, status: 'Draft', locationType: inferNewProposalLocationType(name) }, site: { latitude: Number(business.lat) || 40.391617, longitude: Number(business.lon) || -111.849055 } }, scopes: { ev: true, solar: false, storage: false } }; }); localStorage.setItem(localBidStorageKey, JSON.stringify(copies)); Object.assign(bidProfiles, copies); window.dispatchEvent(new CustomEvent('getev:bulk-proposals-save', { detail: { proposals: copies } })); renderDashboardCardsFromJson(); toast(`${businesses.length} proposal${businesses.length === 1 ? '' : 's'} added to your dashboard.`); });
