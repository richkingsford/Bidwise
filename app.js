...
function mountLayoutMap() {
  ...
  loadLeaflet().then(L => {
    ...
    L.marker([state.site.latitude, state.site.longitude], { icon: L.divIcon({ className: 'layout-site-pin', html: '<span aria-hidden="true"></span>', iconSize: [18, 18], iconAnchor: [9, 9] }) }).addTo(layoutMap).bindPopup(`<b>${esc(state.overview.siteName)}</b><br>${esc(state.overview.location)}`).openPopup();
