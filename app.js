...
const paintRange = input => { const min = Number(input.min), max = Number(input.max), value = Number(input.value); const pct = max === min ? 0 : Math.max(0, Math.min(100, ((value - min) / (max - min)) * 100)); input.style.setProperty('background-image', `linear-gradient(90deg,var(--electric) 0%,var(--electric) ${pct}%,#30323a ${pct}%,#30323a 100%)`, 'important'); input.style.setProperty('background-size', 'calc(100% - 18px) 100%', 'important'); input.style.setProperty('background-position', 'center', 'important'); input.style.setProperty('background-repeat', 'no-repeat', 'important'); };
