...
function renderReferenceComponents() {
  upsertDetail('site', 'reference-components', `<div class="reference-card"><div class="reference-title">UTILITY BASELINE + PROPOSED BILL</div><div class="reference-list"><span><b>Provider</b>${esc(state.site.provider)}</span><span><b>Tariff</b>${esc(state.site.tariff)}</span><span><b>Energy charge</b>${money(state.site.energyRate, 2)} / kWh</span><span><b>Demand charge</b>${money(state.site.demandRate, 2)} / kW-month</span><span><b>Current annual bill</b>${money(state.site.utilitySpend)}</span><span><b>Current monthly bill</b>${money(state.site.utilitySpend / 12)}</span></div><div class="bill-compare"><span>Current bill<strong>${money(state.site.utilitySpend / 12)} / mo</strong></span><i></i><span>Modeled bill<strong>${money(calc.proposedBill() / 12)} / mo</strong></span></div><div class="reference-foot">Baseline uses current utility assumptions until bills and interval data are supplied. The modeled bill excludes financing, taxes, and unvalidated program revenue.</div></div>`);
