...
const paintRange = input => { const min = Number(input.min), max = Number(input.max), value = Number(input.value); const pct = max === min ? 0 : Math.max(0, Math.min(1, (value - min) / (max - min))); const control = input.parentElement; const track = control.querySelector('.config-range-track'); const fill = control.querySelector('.config-range-fill'); if (track) { track.style.cssText = `display:block!important;position:absolute!important;z-index:1;left:9px!important;right:auto!important;top:6px!important;width:${Math.max(0, input.getBoundingClientRect().width - 18)}px!important;height:7px!important;border-radius:8px;background:#30323a;overflow:hidden`; const rangeRect = input.getBoundingClientRect(); const trackRect = track.getBoundingClientRect(); const dialCenter = rangeRect.left + 9 + pct * Math.max(0, rangeRect.width - 18); if (fill) { fill.style.display = 'block'; fill.style.width = `${Math.max(0, dialCenter - trackRect.left)}px`; } } };
function fieldMarkup([key, label, type, options]) {
  ...
  if (type === 'number') { const [min, max, step] = sliderRanges[key] || [0, Math.max(100, Number(value) * 2 || 100), 1]; return `<label class="config-field config-slider-field"><div class="config-slider-head"><span>${esc(label)}</span></div><div class="config-slider-row"><div class="config-range-control"><span class="config-range-track"><i class="config-range-fill"></i></span><input class="config-range" data-key="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${esc(value)}" aria-label="${esc(label)}" /></div><input class="config-number" data-key="${key}" type="number" min="${min}" max="${max}" step="${step}" value="${esc(value)}" aria-label="${esc(label)} value" /></div></label>`; }
