...
const defaults = {
  ...
  vpp: { demandResponse: 12000, reservePct: 20, status: 'Subject to utility approval', controls: 'Secure dispatch + monitoring', customerValue: 'Peak management, resilience, bill control', utilityValue: 'Local capacity and summer peak support', workPlan: 'Metering ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ cybersecurity ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ dispatch testing ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ agreement' },
...
const regionalEvBenchmark = { location: 'St. George, UT', ports: 53, stations: 9, charges3m: 39210, utilization: 18.3, avgKwh: 26.8, sourceWindow: '2026-05-07 to 2026-08-02 Ãƒâ€šÃ‚Â· 88 daily observations' };
...
const configSchemas = {
  overview: { title: 'Overview', fields: [
    ...
    ['Year 1 utility savings', 'Annual utility spend ÃƒÆ’Ã¢â‚¬â€ savings rate', () => money(calc.solarSavings())],
    ['Project investment', 'Solar + battery + EV + site improvements', () => compactMoney(calc.totalInvestment())],
    ['Solar CO2 avoided', 'Annual production ÃƒÆ’Ã¢â‚¬â€ CO2 factor', () => `${number(calc.co2AvoidedSolar(), 0)} t / yr`]
  ] },
  site: { title: 'Site snapshot', fields: [
    ...
    ['Monthly utility bill', 'Annual utility spend ÃƒÆ’Ã‚Â· 12', () => money(state.site.utilitySpend / 12)],
    ['Solar consumed behind meter', 'Solar production ÃƒÆ’Ã¢â‚¬â€ self-consumption', () => `${number(calc.solarMwh() * state.site.selfConsumption / 100, 0)} MWh / yr`], ['Annual energy charges', 'Annual consumption ÃƒÆ’Ã¢â‚¬â€ energy charge', () => money(state.site.annualKwh * state.site.energyRate)], ['Proposed annual utility bill', 'Current bill ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ solar savings ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ demand savings', () => money(calc.proposedBill())], ['Export value spread', 'Onsite energy value ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ export credit', () => money(state.site.onsiteValue - state.site.exportRate, 2) + ' / kWh']
  ] },
  solar: { title: 'Solar array', fields: [
    ...
    ['Annual production', 'Array kW ÃƒÆ’Ã¢â‚¬â€ production ratio ÃƒÆ’Ã‚Â· 1,000', () => `${number(calc.solarMwh(), 0)} MWh / yr`],
    ['Module count', 'CEILING(array kW ÃƒÆ’Ã¢â‚¬â€ 1,000 ÃƒÆ’Ã‚Â· module watts)', () => `${number(calc.modules())} modules`],
    ['Annual energy offset', 'Annual production ÃƒÆ’Ã¢â‚¬â€ 1,000 ÃƒÆ’Ã‚Â· annual consumption', () => `${number(calc.solarMwh() * 1000 / Math.max(1, state.site.annualKwh) * 100, 1)}%`]
  ] },
  storage: { title: 'Battery storage', fields: [
    ...
    ['Post-battery peak', 'Peak demand ÃƒÆ’Ã¢â‚¬â€ (1 ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ shaving target)', () => `${number(calc.postPeak())} kW`],
    ['Annual demand-charge savings', '(Peak demand ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ post-battery peak) ÃƒÆ’Ã¢â‚¬â€ site demand rate ÃƒÆ’Ã¢â‚¬â€ 12', () => money(calc.demandSavings())],
    ['Usable storage', 'Capacity ÃƒÆ’Ã¢â‚¬â€ round-trip efficiency', () => `${number(state.storage.capacityMwh * state.storage.batteryEfficiency / 100, 2)} MWh`]
  ] },
  ev: { title: 'EV customer value', fields: [
    ...
    ['Year 5 charging visits / day', 'Ports ÃƒÆ’Ã¢â‚¬â€ 24 ÃƒÆ’Ã¢â‚¬â€ Year 5 utilization ÃƒÆ’Ã‚Â· average session duration in hours', () => number(calc.evForecastVisits(5), 0)],
    ['Additional parties / day', 'Year 5 visits ÃƒÆ’Ã¢â‚¬â€ restaurant capture rate', () => number(calc.evRoundedParties(5))],
    ['Monthly parties', 'Rounded daily parties ÃƒÆ’Ã¢â‚¬â€ 30', () => number(calc.evRoundedParties(5) * 30)],
    ['Annual expected restaurant sales', 'Rounded daily parties ÃƒÆ’Ã¢â‚¬â€ average receipt ÃƒÆ’Ã¢â‚¬â€ operating days', () => money(calc.evRestaurantSales(state.ev.averageReceipt) * state.ev.daysPerYear)]
  ] },
  bundles: { title: 'Bundled scopes', fields: [
    ...
    ['HVAC coordination / margin', 'HVAC investment ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ HVAC contractor base cost', () => money(state.bundles.hvac - state.bundles.hvacBase)],
...
const sliderRanges = {
  ...
  marketProofVisitsPerDay: [0, 1000, 1], marketProofPorts: [1, 40, 1], ports: [1, 40, 1], averageSessionMinutes: [5, 120, 1], forecastYear1Utilization: [0, 100, 0.1], forecastYear3Utilization: [0, 100, 0.1], forecastYear5Utilization: [0, 100, 0.1], restaurantCaptureRate: [0, 100, 1], averageReceipt: [1, 100, 1], daysPerYear: [1, 366, 1],
...
function refreshLayoutSummary() {
  ...
  count.textContent = `${layoutObjects.length} objects Ãƒâ€šÃ‚Â· ${layoutLines.length} lines`;
...
function addLayoutObject(type, latlng) {
  ...
  const rotate = degrees => { item.rotation = (item.rotation + degrees + 360) % 360; applyRotation(); setLayoutStatus(`${type} rotated to ${item.rotation}Ãƒâ€šÃ‚Â°`); };
  ...
  layoutObjects.push(item); marker.bindTooltip(type, { direction: 'top', offset: [0, -12] }); marker.bindPopup(`<b>${esc(type)}</b><br><span class="layout-popup-angle">Rotation: 0Ãƒâ€šÃ‚Â°</span><br><button type="button" class="layout-rotate-button" data-rotate="-15">ÃƒÂ¢Ã¢â‚¬Â Ã‚Âº 15Ãƒâ€šÃ‚Â°</button> <button type="button" class="layout-rotate-button" data-rotate="15">ÃƒÂ¢Ã¢â‚¬Â Ã‚Â» 15Ãƒâ€šÃ‚Â°</button><br><button type="button" class="layout-delete-button">Delete object</button>`); marker.on('popupopen', event => { const popup = event.popup.getElement(); popup.querySelectorAll('.layout-rotate-button').forEach(button => button.addEventListener('click', () => { rotate(Number(button.dataset.rotate)); popup.querySelector('.layout-popup-angle').textContent = `Rotation: ${item.rotation}Ãƒâ€šÃ‚Â°`; })); popup.querySelector('.layout-delete-button')?.addEventListener('click', remove); }); marker.on('dragend', () => { const pos = marker.getLatLng(); item.lat = pos.lat; item.lng = pos.lng; refreshLayoutSummary(); }); refreshLayoutSummary(); setLayoutStatus(`${type} placed Ãƒâ€šÃ‚Â· click it to rotate, or drag it to refine the location`);
}
function finishLayoutLine(a, b, color) {
  ...
  const feet = Math.round(layoutMap.distance(a, b) * 3.28084); const label = Object.entries({ '#ff5b68': 'Trench', '#ffd166': 'Conduit', '#62a8ff': 'Striping', '#75d58a': 'Access' }).find(([key]) => key === color)?.[1] || 'Line'; const item = { label, feet, line }; const remove = () => { line.remove(); const index = layoutLines.indexOf(item); if (index >= 0) layoutLines.splice(index, 1); refreshLayoutSummary(); setLayoutStatus(`${label} line deleted`); }; layoutLines.push(item); line.bindTooltip(`${label} Ãƒâ€šÃ‚Â· ${feet} ft`, { sticky: true }); line.bindPopup(`<b>${esc(label)} line</b><br>${feet} ft<br><button type="button" class="layout-delete-button">Delete line</button>`); line.on('popupopen', event => event.popup.getElement().querySelector('.layout-delete-button')?.addEventListener('click', remove)); refreshLayoutSummary(); setLayoutStatus(`${label} line drawn Ãƒâ€šÃ‚Â· click two more points for another segment`);
}
function mountLayoutMap() {
  ...
  loadLeaflet().then(L => {
    ...
    L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', { maxZoom: 23, attribution: 'Tiles Ãƒâ€šÃ‚Â© Esri ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Source: Esri, Maxar, Earthstar Geographics' }).addTo(layoutMap);
    ...
    const lineButton = $('#layoutLineTool'); const colorSelect = $('#layoutLineColor'); lineButton?.addEventListener('click', () => { layoutLineActive = !layoutLineActive; lineButton.classList.toggle('active', layoutLineActive); mapElement.classList.toggle('layout-line-mode', layoutLineActive); lineButton.textContent = layoutLineActive ? 'ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Drawing line' : 'ÃƒÂ¯Ã‚Â¼Ã¢â‚¬Â¹ Line'; layoutLinePoints = []; clearLayoutPreview(); setLayoutStatus(layoutLineActive ? 'Pencil active Ãƒâ€šÃ‚Â· click to start, move, then click to commit' : 'Drag objects onto the map Ãƒâ€šÃ‚Â· click Line to draw'); });
    ...
    layoutMap.on('click', event => { if (!layoutLineActive) return; layoutLinePoints.push(event.latlng); if (layoutLinePoints.length === 1) { setLayoutStatus('Pencil active Ãƒâ€šÃ‚Â· move to preview the line, then click to commit'); } if (layoutLinePoints.length === 2) { finishLayoutLine(layoutLinePoints[0], layoutLinePoints[1], colorSelect?.value || state.layout.defaultLineColor); layoutLinePoints = []; } });
    window.bidwiseLayoutMap = layoutMap; setTimeout(() => layoutMap.invalidateSize(), 250);
  }).catch(() => setLayoutStatus('Satellite imagery unavailable Ãƒâ€šÃ‚Â· check the map connection and try again'));
}
function renderDemandMap() {
  ...
  legend.innerHTML = `<b>NEARBY EV DEMAND</b><span>${number(demandStations.length)} locations Ãƒâ€šÃ‚Â· ${number(state.ev.observedPorts)} ports Ãƒâ€šÃ‚Â· ${number(state.ev.observedCharges3m)} charges / 3 mo</span>`;
...
function renderReport() {
  ...
  setText('#storage .battery-copy h3', `${number(state.storage.capacityMwh, 1)} MWh / ${number(state.storage.powerKw)} kW`); setText('#storage .dispatch-footer strong', `ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ ${number(state.storage.shavePct)}% peak demand`); setText('#storage .peak-marker span', `${number(state.site.peakDemand)} kW`);
...
function applyScopeCopy() {
  ...
  const heroSub = $('.hero-sub'); if (heroSub) heroSub.innerHTML = scopes.ev ? `A smarter energy system for <strong id="storeName">${esc(state.overview.siteName)}</strong> ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â designed to reduce operating costs, keep the store resilient, and make EV charging part of the customer experience.` : `A smarter energy system for <strong id="storeName">${esc(state.overview.siteName)}</strong> ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â designed to reduce operating costs and improve the store's energy performance.`;
...
function renderReferenceComponents() {
  ...
  upsertDetail('solar', 'reference-components', `<div class="reference-card"><div class="reference-title">PHASE 1 ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ PHASE 2 ROADMAP</div><div class="roadmap"><span><b>01</b>Build the approved ${number(state.solar.arrayKw)} kW foundation</span><span><b>02</b>Measure 12 months of utility and interval data</span><span><b>03</b>Evaluate additional panels, storage, controls, and VPP enrollment</span></div><div class="reference-foot">System design: ${esc(state.solar.installation)} Ãƒâ€šÃ‚Â· ${esc(state.solar.manufacturer)} ${esc(state.solar.model)} Ãƒâ€šÃ‚Â· ${number(state.solar.productionRatio)} kWh/kW-year.</div></div>`);
  upsertDetail('storage', 'reference-components', `<div class="reference-card"><div class="reference-title">BATTERY VALUE STACK + HOURLY LOAD ANALYSIS</div><div class="reference-list"><span><b>Equipment</b>${number(state.storage.capacityMwh, 1)} MWh rated Ãƒâ€šÃ‚Â· ${number(state.storage.capacityMwh * state.storage.batteryEfficiency / 100, 2)} MWh usable</span><span><b>Manufacturer / model</b>${esc(state.storage.manufacturer)} Ãƒâ€šÃ‚Â· ${esc(state.storage.model)}</span><span><b>Controls</b>${esc(state.storage.controls)}</span><span><b>Energy shifting</b>Solar surplus stored for later store load</span><span><b>Demand reduction</b>${money(calc.demandSavings())} modeled annual demand-charge savings</span><span><b>Resilience</b>${number(state.storage.dispatchHours)}-hour dispatch duration with ${number(state.vpp.reservePct)}% reserve</span></div><table class="load-table"><thead><tr><th>Period</th><th>Store load</th><th>Solar</th><th>Battery</th><th>Grid</th></tr></thead><tbody><tr><td>6 AM</td><td>42%</td><td>8%</td><td>+0%</td><td>34%</td></tr><tr><td>12 PM</td><td>78%</td><td>64%</td><td>+14%</td><td>0%</td></tr><tr><td>6 PM</td><td>92%</td><td>22%</td><td>ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢19%</td><td>51%</td></tr><tr><td>11 PM</td><td>38%</td><td>0%</td><td>ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢8%</td><td>30%</td></tr></tbody></table><div class="load-series"><span>Solar serves load</span><span>Storage charges</span><span>Storage discharges</span><span>Grid imports / exports</span></div></div>`);
  upsertDetail('bundles', 'reference-components', `<div class="reference-card"><div class="reference-title">BUNDLED SCOPE ACCOUNTABILITY</div><div class="scope-detail-grid"><span><b>Critter guard</b>Wiring and roof protection, installation, inspection, and cleanup Ãƒâ€šÃ‚Â· ${money(state.bundles.critterGuard)}</span><span><b>Permanent lighting</b>Low-profile track lighting, controls, Wi-Fi extenders, and electrical work Ãƒâ€šÃ‚Â· ${money(state.bundles.lighting)}</span><span><b>Commercial HVAC</b>Five high-efficiency units, ducting, mechanical/electrical integration Ãƒâ€šÃ‚Â· ${money(state.bundles.hvac)}</span><span><b>Coordination</b>One schedule, documentation package, payment milestones, and first-call accountability Ãƒâ€šÃ‚Â· ${money(state.bundles.coordination)}</span></div><div class="reference-foot">HVAC contractor base: ${money(state.bundles.hvacBase)} Ãƒâ€šÃ‚Â· coordination / margin disclosed: ${money(state.bundles.hvac - state.bundles.hvacBase)}.</div></div>`);
  ...
  upsertDetail('investment', 'reference-components', `<div class="reference-card"><div class="reference-title">SIX-SCOPE SUMMARY + SOLAR TURNKEY BREAKDOWN</div><div class="scope-summary"><span>Solar<strong>${money(state.investment.solar)}</strong></span><span>Battery<strong>${money(state.investment.battery)}</strong></span><span>EV charging<strong>${money(state.investment.ev)}</strong></span><span>Critter guard<strong>${money(state.bundles.critterGuard)}</strong></span><span>Lighting<strong>${money(state.bundles.lighting)}</strong></span><span>HVAC<strong>${money(state.bundles.hvac)}</strong></span></div><div class="reference-list"><span><b>Modules</b>${money(state.investment.solarModules)}</span><span><b>Inverters + monitoring</b>${money(state.investment.solarInverters)}</span><span><b>Commercial racking</b>${money(state.investment.solarRacking)}</span><span><b>Electrical BOS</b>${money(state.investment.solarBos)}</span><span><b>Installation labor</b>${money(state.investment.solarLabor)}</span><span><b>Engineering / approvals</b>${money(state.investment.solarEngineering)}</span><span><b>Delivery / commissioning</b>${money(state.investment.solarCommissioning)}</span></div><div class="tax-grid"><span><b>Credits / incentives</b>Federal clean-energy credit potential, grants, rebates, and stacking review</span><span><b>Depreciation</b>MACRS, bonus depreciation, and Section 179 if applicable</span><span><b>Structure</b>${esc(state.investment.ownership)} Ãƒâ€šÃ‚Â· placed in service ${esc(state.investment.placedInService)}</span><span><b>Documentation</b>Basis, invoices, ownership, timing, and tax-professional review</span></div><div class="comparison-strip"><span>Gross bundled project<strong>${money(calc.totalInvestment())}</strong></span><span>Potential incentive<strong>ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ ${money(calc.totalInvestment() * state.investment.incentivePct / 100)}</strong></span><span>Illustrative net cost<strong>${money(calc.netInvestment())}</strong></span></div><div class="reference-foot">${number(state.investment.incentivePct)}% is an illustrative working assumption, not a guarantee of eligibility or tax outcome.</div></div>`);
...
function addSectionControls(section, sectionId) {
  ...
  const controls = document.createElement('div'); controls.className = 'section-controls'; controls.innerHTML = `<button class="section-edit" type="button" aria-label="Edit ${sectionId} section"><span>ÃƒÂ¢Ã…â€œÃ…Â½</span><b>Edit</b></button><button class="section-config" type="button" aria-label="Configure ${sectionId} section"><span>ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢</span><b>Configure</b></button>`; heading.appendChild(controls);
  const pencil = controls.querySelector('.section-edit'); pencil.addEventListener('click', () => { const editing = pencil.classList.toggle('editing'); pencil.querySelector('span').textContent = editing ? 'ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“' : 'ÃƒÂ¢Ã…â€œÃ…Â½'; editableFor(section).forEach(field => { field.contentEditable = editing; field.classList.toggle('inline-editing', editing); }); });
...
const hero = $('.hero'); const heroActions = document.createElement('div'); heroActions.className = 'hero-actions'; heroActions.innerHTML = '<button class="section-edit" type="button" aria-label="Edit overview"><span>ÃƒÂ¢Ã…â€œÃ…Â½</span><b>Edit</b></button><button class="section-config" type="button" aria-label="Configure overview"><span>ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢</span><b>Configure</b></button>'; hero.appendChild(heroActions);
heroActions.querySelector('.section-edit').addEventListener('click', event => { const editing = event.currentTarget.classList.toggle('editing'); event.currentTarget.querySelector('span').textContent = editing ? 'ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“' : 'ÃƒÂ¢Ã…â€œÃ…Â½'; [hero.querySelector('h1'), hero.querySelector('.hero-sub'), hero.querySelector('.hero-badge strong')].forEach(field => { field.contentEditable = editing; field.classList.toggle('inline-editing', editing); }); });
...
$('#newBidButton')?.addEventListener('click', () => { const toast = $('#toast'); if (toast) { toast.textContent = 'New bid workspace coming next ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â choose an existing bid to begin.'; toast.classList.add('show'); setTimeout(() => toast.classList.remove('show'), 3200); } });
...
updateScopeUI();
if (activeBidId) { renderReport(); setText('.breadcrumb strong', activeBid.locationLabel); document.title = `Bidwise ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${state.overview.siteName} Proposal`; } else { document.title = 'Bidwise ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Sales Workspace'; }
