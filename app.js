...
function updateScopeUI() {
  const scopeControls = $$('.scope-toggle, .config-scope-toggle');
...
$$('.scope-toggle, .config-scope-toggle').forEach(toggle => toggle.addEventListener('change', () => { $$('.scope-toggle, .config-scope-toggle').filter(control => control.dataset.scope === toggle.dataset.scope).forEach(control => { control.checked = toggle.checked; }); updateScopeUI(); applyScopeCopy(); }));
...
$$('.scope-toggle, .config-scope-toggle').forEach(control => { control.checked = activeBid.scopes[control.dataset.scope] !== false; });
