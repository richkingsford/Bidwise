...
function scenarioMarkup(sectionId) { const selected = state.meta.scenarios?.[sectionId] || 'Realistic'; return `<div class="scenario-picker" aria-label="Assumption scenario"><div><b>ASSUMPTION SCENARIO</b><small>Applies a saved range to the editable inputs below.</small></div><div class="scenario-buttons">${scenarioModes.map(mode => `<button type="button" class="scenario-button ${selected === mode ? 'selected' : ''}" data-scenario="${mode}">${mode}</button>`).join('')}</div>${selected === 'Custom' ? '<small class="scenario-custom">Custom inputs saved</small>' : ''}</div>`; }
...
function renderEvSectionInputOutputPanels() {
  Object.entries(evSectionInputGroups).forEach(([index, fields]) => {
    ...
    inputs.innerHTML = fields.length ? `<div class="ev-section-panel-head"><span class="chart-label">ASSUMPTIONS</span><small>Adjust the assumptions that drive this section.</small></div><div class="ev-section-input-scroll">${fields.map(evSectionInputMarkup).join('')}</div>` : `<div class="ev-section-panel-head"><span class="chart-label">ASSUMPTIONS</span><small>This narrative section uses the installer company profile; it has no calculated variables.</small></div>`;
...
function bindEvSectionInputs() {
  $$('.ev-section-input').forEach(input => {
    const update = () => { const sectionName = input.dataset.stateSection, key = input.dataset.stateKey; state[sectionName][key] = Number(input.value); const [min, max] = sliderRanges[key]; input.style.setProperty('--ev-fill', `${Math.max(0, Math.min(100, (Number(input.value) - min) / Math.max(.0001, max - min) * 100))}%`); input.parentElement.querySelector('output').textContent = evSectionInputValue(state[sectionName][key], input.dataset.stateFormat); renderEvCustomerValues(); renderUniversalEvOutputs(); renderEvFinancialValues(); saveState(); };
...
function renderReport() {
  ...
  renderSolarChart(); mountLayoutMap(); setText('.status-pill', state.overview.status);
...
function updateScopeUI() {
  ...
  ['solar', 'storage', 'ev'].forEach(scope => { const section = document.getElementById(scope); const nav = $(`.nav-item[data-target="${scope}"]`); if (section) section.classList.toggle('scope-off', !coreScopes.has(scope)); if (nav) nav.classList.toggle('scope-off', !coreScopes.has(scope)); }); const layoutSection = document.getElementById('layout'); const layoutNav = $('.nav-item[data-target="layout"]'); if (layoutSection) layoutSection.classList.toggle('scope-off', !coreScopes.has('ev')); if (layoutNav) layoutNav.classList.toggle('scope-off', !coreScopes.has('ev')); const bundles = document.getElementById('bundles'); const bundlesNav = $('.nav-item[data-target="bundles"]'); if (bundles) bundles.classList.toggle('scope-off', coreScopes.size !== 3); if (bundlesNav) bundlesNav.classList.toggle('scope-off', coreScopes.size !== 3); const vpp = document.getElementById('vpp'); const vppNav = $('.nav-item[data-target="vpp"]'); if (vpp) vpp.classList.toggle('scope-off', !coreScopes.has('storage')); if (vppNav) vppNav.classList.toggle('scope-off', !coreScopes.has('storage'));
  let number = 2;
