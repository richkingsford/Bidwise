...
const paintRange = input => { const min = Number(input.min), max = Number(input.max), value = Number(input.value); const normalized = max === min ? 0 : Math.max(0, Math.min(1, (value - min) / (max - min))); const trackInset = 9; const width = input.getBoundingClientRect().width || 200; const pct = ((trackInset + normalized * Math.max(0, width - trackInset * 2)) / width) * 100; input.style.setProperty('background-image', `linear-gradient(90deg,var(--electric) 0%,var(--electric) ${pct}%,#30323a ${pct}%,#30323a 100%)`, 'important'); input.style.setProperty('background-size', `calc(100% - ${trackInset * 2}px) 100%`, 'important'); input.style.setProperty('background-position', 'center', 'important'); input.style.setProperty('background-repeat', 'no-repeat', 'important'); };
