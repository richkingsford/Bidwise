...
const refreshBulkBusinesses = async () => { const query = String(bulkForm.elements.location.value || '').trim(); const radius = Math.min(50, Math.max(1, Number(bulkForm.elements.radius.value) || 5)); bulkGenerate.disabled = true; if (query.length < 2) { bulkResults.textContent = 'Enter a city, neighborhood, or address to find businesses.'; return; } bulkResults.textContent = 'Finding businesses…'; try { const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&namedetails=1&limit=10&countrycodes=us&dedupe=1&q=${encodeURIComponent(`${query} businesses`)}`, { headers: { accept: 'application/json' } }); const found = response.ok ? await response.json() : []; bulkBusinesses = Array.isArray(found) ? found.slice(0, 10) : []; bulkResults.innerHTML = bulkBusinesses.length ? `<strong>${bulkBusinesses.length} businesses found within about ${radius} miles</strong><ul>${bulkBusinesses.map(item => `<li>${String(item.display_name || item.name || 'Business').replace(/[&<>]/g, '')}</li>`).join('')}</ul>` : 'No businesses found. Try a city and state, such as “Orem, UT”.'; bulkGenerate.disabled = !bulkBusinesses.length; } catch { bulkBusinesses = []; bulkResults.textContent = 'Business search is temporarily unavailable. Try again in a moment.'; } };
