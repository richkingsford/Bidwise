...
const businessLogoUrl = value => { try { const host = new URL(String(value || '')).hostname; return host ? `https://us-central1-bidwise-production.cloudfunctions.net/businessLogo?url=${encodeURIComponent(value)}` : ''; } catch { return ''; } };
let overtureUtahPlaces = [];
const overtureUtahPlacesReady = fetch('overture-utah-county-places.json', { cache: 'force-cache' }).then(response => response.ok ? response.json() : null).then(data => { overtureUtahPlaces = data?.places || []; return overtureUtahPlaces; }).catch(() => []);
...
const refreshBulkBusinesses = async () => { await overtureUtahPlacesReady; const query = String(bulkForm.elements.location.value || '').trim(); const radius = Math.min(50, Math.max(0.1, Number(bulkForm.elements.radius.value) || 5)); bulkGenerate.disabled = true; if (query.length < 2) { bulkResults.textContent = 'Enter a city, neighborhood, or address to find businesses.'; return; } bulkResults.textContent = 'Finding nearby businesses…'; try { const geoResponse = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&namedetails=1&limit=1&countrycodes=us&q=${encodeURIComponent(query)}`, { headers: { accept: 'application/json' } }); const places = geoResponse.ok ? await geoResponse.json() : []; const center = places[0]; if (!center?.lat || !center?.lon) throw new Error('Location not found'); const meters = Math.round(radius * 1609.34); const overpass = `[out:json][timeout:20];nwr(around:${meters},${center.lat},${center.lon})[name];out center 80;`; const response = await fetch(`https://overpass-api.de/api/interpreter?data=${encodeURIComponent(overpass)}`, { headers: { accept: 'application/json' } }); const payload = response.ok ? await response.json() : { elements: [] }; const distanceMiles=(lat,lon)=>{const dLat=(lat-Number(center.lat))*69;const dLon=(lon-Number(center.lon))*69*Math.cos(Number(center.lat)*Math.PI/180);return Math.sqrt(dLat*dLat+dLon*dLon);}; const overtureElements=overtureUtahPlaces.filter(place=>place.city&&place.state&&place.postcode&&place.address&&distanceMiles(place.lat,place.lon)<=radius).map(place=>({name:place.name,description:place.category.replace(/[-_]/g,' '),display_name:[place.name,place.address,place.city,place.state,place.postcode].filter(Boolean).join(', '),lat:place.lat,lon:place.lon,website:place.website||'',address:{house_number:(place.address.match(/^\d+[A-Za-z-]*/) || [''])[0],road:place.address.replace(/^\d+[A-Za-z-]*\s*/,'').trim(),city:place.city,state:place.state,postcode:place.postcode}})); const elements=overtureElements.length ? overtureElements : (payload.elements || []); const appMatches = !elements.length && typeof window.getevSearchLocationSuggestions === 'function' ? await window.getevSearchLocationSuggestions(query) : []; const fallbackResponse = elements.length ? null : await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&namedetails=1&limit=20&countrycodes=us&q=${encodeURIComponent(query)}`, { headers: { accept: 'application/json' } }); const fallback = fallbackResponse?.ok ? await fallbackResponse.json() : []; const seen = new Set(); bulkBusinesses = (elements.length ? elements.map(item => { const tags = item.tags || {}; const isOverture = !item.tags && item.name; const point = isOverture ? { lat: item.lat, lon: item.lon } : (item.type === 'node' ? { lat: item.lat, lon: item.lon } : item.center || {}); const category = isOverture ? item.description : (tags.amenity || tags.shop || tags.office || tags.tourism || 'business'); const description = String(category).replace(/[-_]/g, ' ').replace(/\b\w/g, letter => letter.toUpperCase()); return { name: isOverture ? item.name : tags.name, description, display_name: isOverture ? item.display_name : [tags.name, [tags['addr:housenumber'], tags['addr:street']].filter(Boolean).join(' '), tags['addr:city'], tags['addr:state'], tags['addr:postcode']].filter(Boolean).join(', '), lat: point.lat, lon: point.lon, logoUrl: isOverture ? (item.website ? businessLogoUrl(item.website) : '') : (tags.website ? businessLogoUrl(tags.website) : ''), address: isOverture ? item.address : { house_number: tags['addr:housenumber'], road: tags['addr:street'], city: tags['addr:city'], state: tags['addr:state'], postcode: tags['addr:postcode'] } }; }) : (appMatches.length ? appMatches : (Array.isArray(fallback) ? fallback : []))).filter(item => item.name && item.address?.house_number && item.address?.road && (item.address?.city || item.address?.town || item.address?.village) && (item.address?.state_code || item.address?.state) && item.address?.postcode && !seen.has(item.name.toLowerCase()) && seen.add(item.name.toLowerCase())).slice(0, 10); const cleanAddress = item => { const address = item.address || {}; return [address.house_number && address.road ? [address.house_number, address.road].join(' ') : address.road, address.city || address.town || address.village, address.state_code || address.state, address.postcode].filter(Boolean).join(', ') || item.display_name || 'Address not listed'; }; bulkResults.innerHTML = bulkBusinesses.length ? `<strong>${bulkBusinesses.length} real businesses found within about ${radius} miles</strong><div class="bulk-business-list">${bulkBusinesses.map((item, index) => `<label class="bulk-business-row"><input type="checkbox" name="businessIndex" value="${index}" checked><span><b>${item.logoUrl ? `<img class=\"bulk-business-logo\" src=\"${String(item.logoUrl).replace(/[&<>\"']/g, '')}\" alt=\"\" onerror=\"this.hidden=true\">` : ''}${String(item.name).replace(/[&<>]/g, '')}</b><small>${String(item.description || 'Local business').replace(/[&<>]/g, '')} · ${String(cleanAddress(item)).replace(/[&<>]/g, '')}</small></span><input class=\"bulk-business-logo-url\" type=\"url\" value=\"${String(item.logoUrl || '').replace(/[&<>\"']/g, '')}\" placeholder=\"Logo URL (optional)\" data-business-logo-url=\"${index}\"></label>`).join('')}</div>` : 'No named businesses found in that radius. Try a nearby city or increase the radius.'; bulkGenerate.disabled = !bulkBusinesses.length; } catch { bulkBusinesses = []; bulkResults.textContent = 'We could not find businesses there. Try a city and state, such as “Orem, UT”, or increase the radius.'; } };
bulkResults.addEventListener('input', event => { const input=event.target.closest('[data-business-logo-url]'); if(input && bulkBusinesses[Number(input.dataset.businessLogoUrl)]) bulkBusinesses[Number(input.dataset.businessLogoUrl)].logoUrl=input.value.trim(); }); let bulkTimer; ['location', 'radius'].forEach(name => bulkForm.elements[name].addEventListener('input', () => { clearTimeout(bulkTimer); bulkTimer = setTimeout(refreshBulkBusinesses, 350); }));
...
bulkForm.addEventListener('submit', event => { event.preventDefault(); const count = Math.min(10, Math.max(1, Number(bulkForm.elements.count.value) || 1)); const selected = [...bulkForm.querySelectorAll('input[name="businessIndex"]:checked')].map(input => bulkBusinesses[Number(input.value)]).filter(Boolean).slice(0, count); window.dispatchEvent(new CustomEvent('getev:bulk-proposals', { detail: { businesses: selected, radius: Number(bulkForm.elements.radius.value) || 5 } })); closeBulk(); });
...
const showCompanyModal = (user, profile = {}) => {
  ...
  // A profile can be completed gradually. Never let native validation make Save appear inert.
  ['companyName', 'contactName', 'email', 'territory'].forEach(name => companyForm.elements[name]?.removeAttribute('required'));
  ...
  companyForm.elements.companyLogoUrl.value = typeof profile.companyLogo === 'string' ? profile.companyLogo : profile.companyLogo?.url || '';
  companyForm.elements.companyPhotoUrl.value = typeof profile.companyPhoto === 'string' ? profile.companyPhoto : profile.companyPhoto?.url || '';
...
companyForm?.addEventListener('submit', async event => {
  ...
  const insuranceFile = companyForm.elements.insuranceDocument?.files?.[0]; const contractorFile = companyForm.elements.contractorCertification?.files?.[0]; const logoFile = companyForm.elements.companyLogo?.files?.[0]; const photoFile = companyForm.elements.companyPhoto?.files?.[0]; const logoUrl = String(form.get('companyLogoUrl') || '').trim(); const photoUrl = String(form.get('companyPhotoUrl') || '').trim();
  ...
  try { const [insuranceDocument, contractorCertification, companyLogo, companyPhoto] = await Promise.all([uploadCompanyDocument(insuranceFile), uploadCompanyDocument(contractorFile), uploadCompanyDocument(logoFile || logoUrl, 'company-logos'), uploadCompanyDocument(photoFile || photoUrl, 'company-photos')]); const certificationDocuments = []; for (const file of [...(companyForm.elements.certificationDocuments?.files || [])]) certificationDocuments.push(await uploadCompanyDocument(file)); const pendingStorage = [insuranceDocument, contractorCertification, companyLogo, companyPhoto, ...certificationDocuments].some(document => document?.storageStatus === 'pending-storage-setup'); if (typeof saveCompanyProfile !== 'function') throw new Error('Profile storage is not ready. Refresh the page and try again.'); const usableMedia = (media, existing) => media?.url ? media : existing || null; await saveCompanyProfile({ companyName, tagline: String(form.get('tagline') || '').trim(), proposalSlogan: String(form.get('proposalSlogan') || '').trim(), contactName, businessEmail, territory, website: String(form.get('website') || '').trim(), services, proposalCertifications: String(form.get('proposalCertifications') || '').trim(), companyLogo: usableMedia(companyLogo, currentProfile?.companyLogo), companyPhoto: usableMedia(companyPhoto, currentProfile?.companyPhoto), insuranceDocument: insuranceDocument || currentProfile?.insuranceDocument || null, contractorCertification: contractorCertification || currentProfile?.contractorCertification || null, certificationDocuments: certificationDocuments.length ? certificationDocuments : currentProfile?.certificationDocuments || [], documentStorageStatus: pendingStorage ? 'pending-storage-setup' : 'stored' }); if (companyFormNote) companyFormNote.textContent = pendingStorage ? 'Saved. Your profile is updated; a file upload is still awaiting secure storage.' : 'Saved. Your profile branding is now applied to Slide 5.'; submit.textContent = 'Saved'; setTimeout(() => { hideCompanyModal(); toast(pendingStorage ? 'Company profile saved. One or more file uploads are still pending.' : 'Company profile saved. Slide 5 has been updated.'); }, 750); } catch (error) { console.error('GetEV company profile save error', error); const saveError = `Could not save your company profile${error?.code ? ` (${error.code})` : ''}. Please try again.`; if (companyFormNote) companyFormNote.textContent = saveError; toast(saveError); } finally { setTimeout(() => { submit.disabled = false; submit.textContent = currentProfile?.companyName ? 'Save company profile' : 'Create company profile'; }, 800); }
});
