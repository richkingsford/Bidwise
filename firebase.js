...
if (firebaseConfig && !isLocalFile) {
  ...
  const uploadCompanyDocument = async file => { if (!file) return null; const metadata = { name: file.name, type: file.type, size: file.size }; try { const safeName = file.name.replace(/[^a-z0-9._-]+/gi, '-'); const target = ref(storage, `company-documents/${currentUser.uid}/${Date.now()}-${safeName}`); const uploaded = await uploadBytes(target, file, { contentType: file.type }); return { ...metadata, url: await getDownloadURL(uploaded.ref), storageStatus: 'stored' }; } catch (error) { console.warn('GetEV document storage pending', error); return { ...metadata, storageStatus: 'pending-storage-setup' }; } };
  ...
  const renderAdminProfiles = profiles => {
    ...
    adminProfilesList.innerHTML = profiles.map(profile => `<article class="admin-profile"><div><strong>${escapeHtml(profile.companyName || 'Unnamed company')}</strong><span>${escapeHtml(profile.businessEmail || profile.email || 'No email')} Â· ${escapeHtml(profile.territory || 'Territory not provided')}</span><small>${escapeHtml(Object.entries(profile.services || {}).filter(([, enabled]) => enabled).map(([key]) => key === 'storage' ? 'Battery storage' : key === 'ev' ? 'EV charging' : 'Solar').join(' Â· ') || 'No services selected')}</small></div><div class="admin-profile-actions"><span class="admin-status ${escapeHtml(profile.verificationStatus || 'pending')}">${escapeHtml(profile.verificationStatus || 'pending')}</span><button type="button" data-admin-status="approved" data-profile-id="${escapeHtml(profile.id)}">Approve</button><button type="button" data-admin-status="rejected" data-profile-id="${escapeHtml(profile.id)}">Reject</button></div></article>`).join('');
  ...
  const explainAuthError = error => {
    ...
    if (error?.code === 'auth/unauthorized-domain') {
      toast('This site is not authorized in Firebase yet. Add richkingsford.github.io under Authentication â†’ Settings â†’ Authorized domains.');
      return;
    }
    if (error?.code === 'auth/popup-blocked' || error?.code === 'auth/operation-not-supported-in-this-environment') {
      toast('Opening Google sign-in in this tabâ€¦');
      return;
    }
  ...
  onAuthStateChanged(auth, async user => {
    ...
    if (!user.email?.toLowerCase().endsWith('@gmail.com')) { await signOut(auth); toast('Please use a Gmail address to access GetEV.'); return; }
    setIdentity(user);
    try {
